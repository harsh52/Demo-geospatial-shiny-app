...
server <- function(input, output, session) {
    zone.rg <- readOGR(dsn="D:\\freelance\\Map-Freight-Analysis-Framework\\FAF_Zones\\faf4_zone2.shp",layer="faf4_zone2", encoding = "UTF-8")
    ...
    output$Zone <- renderLeaflet({
      ...
      m<-leaflet() %>%
        ...
        addPolygons(data=zone.rg, col="yellow", weight = 1, layerId = ~id, label = zone_labels,
